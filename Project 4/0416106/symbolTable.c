...
TableEntry* FindEntryInRange (SymbolTable* table, char* name) {
	...
	for (level = table->current_level; level >= 0; --level) {	
		for (i = 0; i < table->data_count; ++i) {
			TableEntry* entry = table->table_entry[i];
			if(strcmp(name, entry->name) == 0 && entry->level == level && strcmp(entry->kind, "loop variable") != 0 && strcmp(entry->kind, "function") != 0) {
				return entry;
			}
		}
	}
...
Expr* FunctionCall (char* name, ExprList* list) {
	...
	if (CheckParameterNumber(exp)) {
		if (exp->para == NULL) return exp;
		for (i = 0; i < exp->para->data_count; ++i) {
...
void CheckFileName (char* file, char* name) {
	...
	if(strcmp(p, name) != 0) printf("<Error> found in Line %d: program ID inconsistent with file name\n", linenum);
	return;
}
...
void CheckAssignmentType (Expr* left, Expr* right) {
	if (left == NULL || right == NULL) return;
	if (strcmp(left->kind, "err") == 0 || strcmp(right->kind, "err") == 0) return;
	char* left_type = GetType(left->type, left->current_dimension);
	char* right_type = GetType(right->type, right->current_dimension);
...
void AssignmentCheck (Expr* left, Expr* right) {
	if (CheckConstAssign(left)) CheckAssignmentType(left, right);
	return;
}
...
int CheckParameterNumber (Expr* exp) {
	...
	if (strcmp(exp->type->name, "void") == 0 && exp->para == NULL) return 1;
	if (strcmp(exp->type->name, "void") == 0 && exp->para != NULL) {
		printf("<Error> found in Line %d: void function '%s' has no parameter\n", linenum, exp->name);
		return 0;
	}
	if (strcmp(exp->type->name, "void") != 0 && exp->para == NULL) {
